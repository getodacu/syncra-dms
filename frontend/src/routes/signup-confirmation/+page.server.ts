		...
		try {
			...
			if (isAuthApiError(error)) {
				return fail(publicErrorStatus(error.status), {
					values: { email, otp: '' },
					error: publicErrorMessage(error.status, error.message, 'Unable to verify email. Please try again.')
				});
			}
			throw error;
		}
		redirect(303, `/login?${new URLSearchParams({ email, verified: '1' })}`);
	},
	resend: async ({ fetch, locals, request }) => {
		const data = await request.formData();
		const email = textValue(data, 'email').trim().toLowerCase();
		if (!email) return fail(400, { values: { email }, fieldErrors: { email: 'Email is required.' } });
		...
		try {
			...
			if (isAuthApiError(error)) {
				return fail(publicErrorStatus(error.status), {
					values: { email },
					error: publicErrorMessage(error.status, error.message, 'Unable to send verification code. Please try again.')
				});
			}
			throw error;
		}
		if (verificationCode) {
			...
			try {
				await sendVerificationEmail({ to: email, code: verificationCode, expiresAt: verificationExpiresAt });
			} catch {
				return fail(502, { values: { email }, error: VERIFICATION_EMAIL_DELIVERY_ERROR });
			}
		}
		return { values: { email }, success: 'A new verification code has been sent. Check your inbox and spam folder.' };
	}
} satisfies Actions;
