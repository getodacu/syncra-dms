	...
	request: async ({ fetch, locals, request, url }) => {
		...
		try {
			...
			if (isAuthApiError(error)) {
				return fail(publicErrorStatus(error.status), {
					values: { email },
					error: publicErrorMessage(error.status, error.message, 'Unable to request password reset. Please try again.')
				});
			}
			throw error;
		}
		if (resetToken) {
			try {
				...
				locals.logger?.error?.('auth.password_reset_email_failed', { email, error: String(error) });
			}
		}
		return { values: { email }, success: PASSWORD_RESET_REQUEST_SUCCESS };
	},
	reset: async ({ fetch, request }) => {
		...
		try {
			...
			if (isAuthApiError(error)) {
				return fail(publicErrorStatus(error.status), {
					values,
					error: publicErrorMessage(error.status, error.message, 'Unable to reset password. Please try again.')
				});
			}
			throw error;
		}
		redirect(303, `/login?${new URLSearchParams({ email, reset: '1' })}`);
	}
} satisfies Actions;
