...
export const actions = {
	default: async ({ fetch, locals, request }) => {
		...
		try {
			...
			if (isAuthApiError(error)) {
				return fail(publicErrorStatus(error.status), {
					values,
					error: publicErrorMessage(error.status, error.message, 'Unable to create account. Please try again.')
				});
			}
			throw error;
		}
		...
		if (verificationCode) {
			...
			try {
				await sendVerificationEmail({ to: email, name, code: verificationCode, expiresAt: verificationExpiresAt });
			} catch {
				return fail(502, { values, error: VERIFICATION_EMAIL_DELIVERY_ERROR });
			}
		}
		redirect(303, `/signup-confirmation?${new URLSearchParams({ email })}`);
	}
} satisfies Actions;
